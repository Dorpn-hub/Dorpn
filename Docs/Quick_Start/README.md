...
Colons (`:`) are now required at every block header, indentation must be a consistent 2 or 4 spaces, and `String32`/`Bool32` have been removed in favor of `String`/`Bool`. If you're copying these examples into an older Dorpn install, they may not run correctly.
